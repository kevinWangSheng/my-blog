...
知名度比较高的就是 tonic(完整正式版还没有发布,看相关开发人说 26 年发布第一版)

github: https://github.com/hyperium/tonic

文档:https://docs.rs/tonic/latest/tonic/
...
1. 需要添加基础的依赖(调用依赖 tonic, 生成代码依赖 tonic_prots_build, 协议依赖: prost)
对应的 toml 依赖
...
[[bin]]
name = "client"    # 改别名成为client ,直接cargo run --bin client 就可以运行(下面同样的)
path = "src/client.rs"
...
tonic = "0.14.2"
tonic-prost = "0.14.2"
[build-dependencies]  # 编译的时候使用到的 (cargo build)
...
1. 需要编写对应的 proto 
可以参考: https://protobuf.dev/programming-guides/proto3/
...
然后进行链接,需要编写对应的 build.rs(编译时会生成对应的代码,通过 tonic-prost-build 生成对应的 rs 代码), 这个文件必须要放到跟你对应的 cargo 运行的目录同级(即 Cargo.toml 同级),这里就是根目录
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    tonic_prost_build::configure()
        .out_dir("src/generate") // 生成文件输出的目录,如果不指定,他会默认生成在对应的 target 目录下的对应路径,后面你需要通过 tonic::include_proto!()宏进行引入
        .compile_protos(&["proto/hello.proto", "proto/test.proto"], &["proto/"])?;  // 这里是你写的对应的 proto 文件内容
    Ok(())
}

```

对应你运行 cargo build 之后会生成对应的内容:
...
impl Greeter for MyGreeter {
    async fn say_hello(&self,request :Request<HelloRequest>)->Result<Response<HelloResponse>, tonic::Status>{
        ...
        let response = HelloResponse {
            message : format!( "Hello, World, this is from the grpc server 🦀:{}",request.into_inner().name)
        };
        Ok(Response::new(response))
    }
...
async fn main() -> Result<(),Box<dyn std::error::Error>> {
    ...
    Server::builder().add_service(GreeterServer::new(server)).add_service(TestServer::new(test)).serve(addr).await?; // 进行对应的 rpc 的绑定
    Ok(())
}
```
...
📤 测试一元 RPC:
✅ 收到响应: Hello, World, this is from the grpc server 🦀:Shawn

📤 测试 Server Streaming:
📨 收到流式响应: this is from the grpc server message :0
📨 收到流式响应: this is from the grpc server message :1
📨 收到流式响应: this is from the grpc server message :2
📨 收到流式响应: this is from the grpc server message :3
📨 收到流式响应: this is from the grpc server message :4
...
1. 总结: 简单使用场景下,需要 链接,生成,在配置 server,总体的使用方式不是非常友好,流程也相对复杂. 
